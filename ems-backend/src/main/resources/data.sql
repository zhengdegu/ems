-- EMS 设备管理系统 - 初始数据 (MySQL)

-- 默认管理员账号 (密码: admin123，BCrypt加密)
INSERT IGNORE INTO sys_user (username, password, name, role, enabled) VALUES
('admin', '$2a$10$N.zmdr9k7uOCQb376NoUnuTJ8iAt6Z5EHsM8lE9lBOsl7iKTVKIUi', '系统管理员', 'admin', 1);

-- 初始角色
INSERT IGNORE INTO role (name, code, description, enabled) VALUES
('管理员', 'admin', '系统管理员，拥有所有权限', 1),
('普通用户', 'user', '普通用户，拥有基本操作权限', 1),
('维护人员', 'maintainer', '维护人员，负责设备维护和工单处理', 1),
('设备主管', 'supervisor', '设备部门主管', 1),
('维修工程师', 'engineer', '负责设备维修', 1),
('操作员', 'operator', '设备操作人员', 1);

-- 初始设备
INSERT IGNORE INTO equipment (code, name, type, model, manufacturer, manufacture_date, location, responsible, status, health_score) VALUES
...
-- 初始工单
INSERT IGNORE INTO work_order (code, title, type, equipment_id, equipment_name, priority, assignee, status, description) VALUES
...
-- 初始告警
INSERT IGNORE INTO alarm (equipment_id, equipment_name, level, message, status) VALUES
...
-- 初始维护计划
INSERT IGNORE INTO maintenance_plan (name, type, equipment_id, equipment_name, cycle, next_date, responsible, status) VALUES
...
-- 默认系统设置
INSERT IGNORE INTO system_setting (setting_key, setting_value, label, description) VALUES
